...
export class AuthService {
  ...
  logout(): Observable<any> {
    ...
    return this.http.post<any>(`http://localhost:5025/api/users/logout`, userId).pipe(
      tap(() => {
  ...
  getRecentUsers(limit: number = 50): Observable<any> {
    return this.http.get(`http://localhost:5025/api/users/recent/${limit}`, {
      headers: this.getAuthHeaders()
    });
  }

  toggleUserStatus(userId: string, isActive: boolean): Observable<any> {
    return this.http.put(`http://localhost:5025/api/users/${userId}/status`, { isActive }, {
      headers: this.getAuthHeaders()
    });
  }

  deleteUser(userId: string): Observable<any> {
    return this.http.delete(`http://localhost:5025/api/users/${userId}`, {
      headers: this.getAuthHeaders()
    });
  }

  getUserById(userId: string): Observable<any> {
    return this.http.get(`http://localhost:5025/api/users/${userId}`, {
      headers: this.getAuthHeaders()
    });
  }
