...
export class AuthService {
  private apiUrl = 'https://edulearn-auth-service-d960.onrender.com/api/auth';
  ...
  logout(): Observable<any> {
    ...
    return this.http.post<any>(`https://edulearn-auth-service-d960.onrender.com/api/users/logout`, userId).pipe(
      tap(() => {
  ...
  getRecentUsers(limit: number = 50): Observable<any> {
    return this.http.get(`https://edulearn-auth-service-d960.onrender.com/api/users/recent/${limit}`, {
      headers: this.getAuthHeaders()
    });
  }

  toggleUserStatus(userId: string, isActive: boolean): Observable<any> {
    return this.http.put(`https://edulearn-auth-service-d960.onrender.com/api/users/${userId}/status`, { isActive }, {
      headers: this.getAuthHeaders()
    });
  }

  deleteUser(userId: string): Observable<any> {
    return this.http.delete(`https://edulearn-auth-service-d960.onrender.com/api/users/${userId}`, {
      headers: this.getAuthHeaders()
    });
  }

  getUserById(userId: string): Observable<any> {
    return this.http.get(`https://edulearn-auth-service-d960.onrender.com/api/users/${userId}`, {
      headers: this.getAuthHeaders()
    });
  }
